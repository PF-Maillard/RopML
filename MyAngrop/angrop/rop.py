...
def DataNormalization(X, Vectorizer):
    Maxlen  = 0
    ...
    for j in range(len(X)):
        for i in range(len(Vectorizer.feature_names_out_)):
...
# global initializer for multiprocessing
def _set_global_gadget_analyzer_ML(rop_gadget_analyzer):
    global _global_gadget_analyzer
    _global_gadget_analyzer = rop_gadget_analyzer
    _disable_loggers()
...
def run_worker_ML(addr):
    return _global_gadget_analyzer.analyze_gadget_with_ML(addr)
...
class ROP(Analysis):
    ...
    def DeleteBadBlockML(self, GenAddress, Classifier, Vectorizer, Coef):
        NewListAddress = []
        ListGadget = []
        
        print("ANALYZE ML")

        ListAddress = list(GenAddress)

        for addr in ListAddress:
            NewGadget = {}
            if not self._gadget_analyzer._block_makes_sense(addr):
                ListGadget.append(NewGadget)
                continue

            if  self._gadget_analyzer._does_not_get_to_unconstrained(addr):
                ListGadget.append(NewGadget)
                continue

            MyBlock = self._gadget_analyzer.block
            Gadget = MyBlock.split("\n")
            for j in range(len(Gadget)):
                NewGadget["Instruction_" + str(len(Gadget) - j - 1)] = FormatInstruction(GoodFormat(Gadget[j]))
            #print(NewGadget)
            ListGadget.append(NewGadget)
        
        Xt = DataNormalization(ListGadget, Vectorizer)           
        Xt = DictToNPList(Xt)

        X = Vectorizer.transform(Xt)
        ClassifierResult = Classifier.predict_proba(X)

        for i in range(len(ListAddress)):
            if ClassifierResult[i][1] >= Coef:  
                NewListAddress.append(ListAddress[i])

        print(str((1 - len(NewListAddress)/len(ListAddress))*100) + "% Gadgets removed")

        GenNewAddress = (Gadget for Gadget in NewListAddress)

        return GenNewAddress
    ...
    def find_gadgets_without_analysis(self):
        MyListGadgets = []
        self._gadgets = []

        self._initialize_gadget_analyzer()

        for  _, addr in enumerate(self._addresses_to_check_with_caching(True)):
            if self._gadget_analyzer._block_makes_sense(addr):
                if not self._gadget_analyzer._does_not_get_to_unconstrained(addr):
                    MyBlock = self._gadget_analyzer.block
                    MyListGadgets.append(MyBlock)

        return MyListGadgets
    ...
    def find_gadgets_with_ML(self, Classifier, Vectorizer, Coef = 0.05, processes=4, show_progress=True):
        """
        Finds all the gadgets in the binary by calling analyze_gadget on every address near a ret.
        Saves gadgets in self._gadgets
        Saves stack pivots in self.stack_pivots
        ...
        pool = Pool(processes=processes, initializer=_set_global_gadget_analyzer_ML, initargs=(self._gadget_analyzer,))
        ListAddress = self._addresses_to_check_with_caching(show_progress)

        ListAddress = self.DeleteBadBlockML(ListAddress, Classifier, Vectorizer, Coef)

        it = pool.imap_unordered(run_worker_ML, ListAddress, chunksize=5)
        for gadget in it:
            if gadget is not None:
                if isinstance(gadget, RopGadget):
                    self._gadgets.append(gadget)
